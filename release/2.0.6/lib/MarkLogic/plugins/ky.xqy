...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ky", xdmp:function(xs:QName("dbl10n:ky")))
    return $map
};

declare function dbl10n:ky()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ky.xqy"),
 plugin:register(dbl10n:capabilities(),"ky.xqy"))
