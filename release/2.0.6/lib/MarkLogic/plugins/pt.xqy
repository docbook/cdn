...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/pt", xdmp:function(xs:QName("dbl10n:pt")))
    return $map
};

declare function dbl10n:pt()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/pt.xqy"),
 plugin:register(dbl10n:capabilities(),"pt.xqy"))
