...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ko", xdmp:function(xs:QName("dbl10n:ko")))
    return $map
};

declare function dbl10n:ko()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ko.xqy"),
 plugin:register(dbl10n:capabilities(),"ko.xqy"))
