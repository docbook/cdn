...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/lt", xdmp:function(xs:QName("dbl10n:lt")))
    return $map
};

declare function dbl10n:lt()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/lt.xqy"),
 plugin:register(dbl10n:capabilities(),"lt.xqy"))
