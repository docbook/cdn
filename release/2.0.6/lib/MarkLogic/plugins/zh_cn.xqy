...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/zh_cn", xdmp:function(xs:QName("dbl10n:zh_cn")))
    return $map
};

declare function dbl10n:zh_cn()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/zh_cn.xqy"),
 plugin:register(dbl10n:capabilities(),"zh_cn.xqy"))
