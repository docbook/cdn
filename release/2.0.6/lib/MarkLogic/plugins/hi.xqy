...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/hi", xdmp:function(xs:QName("dbl10n:hi")))
    return $map
};

declare function dbl10n:hi()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/hi.xqy"),
 plugin:register(dbl10n:capabilities(),"hi.xqy"))
