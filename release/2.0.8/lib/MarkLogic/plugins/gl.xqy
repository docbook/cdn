...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/gl", xdmp:function(xs:QName("dbl10n:gl")))
    return $map
};

declare function dbl10n:gl()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/gl.xqy"),
 plugin:register(dbl10n:capabilities(),"gl.xqy"))
