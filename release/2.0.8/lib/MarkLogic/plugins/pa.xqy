...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/pa", xdmp:function(xs:QName("dbl10n:pa")))
    return $map
};

declare function dbl10n:pa()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/pa.xqy"),
 plugin:register(dbl10n:capabilities(),"pa.xqy"))
