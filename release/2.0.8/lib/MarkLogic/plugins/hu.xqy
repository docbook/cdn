...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/hu", xdmp:function(xs:QName("dbl10n:hu")))
    return $map
};

declare function dbl10n:hu()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/hu.xqy"),
 plugin:register(dbl10n:capabilities(),"hu.xqy"))
