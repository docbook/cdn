...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/sr", xdmp:function(xs:QName("dbl10n:sr")))
    return $map
};

declare function dbl10n:sr()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/sr.xqy"),
 plugin:register(dbl10n:capabilities(),"sr.xqy"))
