...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ga", xdmp:function(xs:QName("dbl10n:ga")))
    return $map
};

declare function dbl10n:ga()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ga.xqy"),
 plugin:register(dbl10n:capabilities(),"ga.xqy"))
