...
function checkTime(i) {
  ...
  if (timer) {
    var prefix = "";
    var secs = Math.floor((nowDT.getTime() - startDT.getTime()) / 1000) - timerofs;
    if (countdown > 0) {
      secs = countdown - secs;
...
function navigate(code) {
  ...
  if ((code == ARR_R || code == ARR_D || code == KEY_n || code == KEY_space || code == PGDN)) {
    reveal_next(curSlide, "slow", curSlide < totSlides);
  }

  if ((code == ARR_L || code == ARR_U || code == KEY_p || code == PGUP) && curSlide > 1) {
    goto(curSlide-1);
  }
