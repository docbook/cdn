...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ja", xdmp:function(xs:QName("dbl10n:ja")))
    return $map
};

declare function dbl10n:ja()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ja.xqy"),
 plugin:register(dbl10n:capabilities(),"ja.xqy"))
