...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/hr", xdmp:function(xs:QName("dbl10n:hr")))
    return $map
};

declare function dbl10n:hr()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/hr.xqy"),
 plugin:register(dbl10n:capabilities(),"hr.xqy"))
