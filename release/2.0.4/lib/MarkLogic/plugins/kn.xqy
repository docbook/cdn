...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/kn", xdmp:function(xs:QName("dbl10n:kn")))
    return $map
};

declare function dbl10n:kn()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/kn.xqy"),
 plugin:register(dbl10n:capabilities(),"kn.xqy"))
