...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ro", xdmp:function(xs:QName("dbl10n:ro")))
    return $map
};

declare function dbl10n:ro()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ro.xqy"),
 plugin:register(dbl10n:capabilities(),"ro.xqy"))
