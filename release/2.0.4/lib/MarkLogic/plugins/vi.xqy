...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/vi", xdmp:function(xs:QName("dbl10n:vi")))
    return $map
};

declare function dbl10n:vi()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/vi.xqy"),
 plugin:register(dbl10n:capabilities(),"vi.xqy"))
