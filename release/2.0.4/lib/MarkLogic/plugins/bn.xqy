...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/bn", xdmp:function(xs:QName("dbl10n:bn")))
    return $map
};

declare function dbl10n:bn()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/bn.xqy"),
 plugin:register(dbl10n:capabilities(),"bn.xqy"))
