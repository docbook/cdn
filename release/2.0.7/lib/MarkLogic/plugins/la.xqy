...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/la", xdmp:function(xs:QName("dbl10n:la")))
    return $map
};

declare function dbl10n:la()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/la.xqy"),
 plugin:register(dbl10n:capabilities(),"la.xqy"))
