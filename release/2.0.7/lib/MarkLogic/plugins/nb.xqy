...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/nb", xdmp:function(xs:QName("dbl10n:nb")))
    return $map
};

declare function dbl10n:nb()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/nb.xqy"),
 plugin:register(dbl10n:capabilities(),"nb.xqy"))
