...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/mn", xdmp:function(xs:QName("dbl10n:mn")))
    return $map
};

declare function dbl10n:mn()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/mn.xqy"),
 plugin:register(dbl10n:capabilities(),"mn.xqy"))
