...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/sl", xdmp:function(xs:QName("dbl10n:sl")))
    return $map
};

declare function dbl10n:sl()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/sl.xqy"),
 plugin:register(dbl10n:capabilities(),"sl.xqy"))
