...
<xsl:template name="generate-index">
  ...
  <xsl:param name="scope" select="(ancestor::book                                   | ancestor::appendix                                   | ancestor::article                                   | ancestor::chapter                                   | ancestor::part                                   | ancestor::preface                                   | ancestor::sect1                                   | ancestor::sect2                                   | ancestor::sect3                                   | ancestor::sect4                                   | ancestor::sect5                                   | ancestor::section                                   | ancestor::set                                  | ancestor::topic                                   |/)[last()]"/>
...
<xsl:template name="generate-basic-index">
  ...
  <div class="index">
    <xsl:if test="$others">
      ...
      <xsl:variable name="others.in.index" select="$others[generate-id(.) = generate-id(key('primary', normalize-space(concat(primary/@sortas, &#34; &#34;, primary)))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]"/>
      <div class="indexdiv">
        <h3>
...
<xsl:template match="indexterm" mode="index-div-basic">
  ...
  <xsl:if test="key('letter', $key)                 [generate-id(.) = generate-id(key('primary', normalize-space(concat(primary/@sortas, &#34; &#34;, primary)))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]">
    <div class="indexdiv">
      ...
      <dl>
        <xsl:variable name="these.terms" select="key('letter', $key)                               [generate-id(.) = generate-id(key('primary', normalize-space(concat(primary/@sortas, &#34; &#34;, primary)))                               [count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]"/>
...
<xsl:template match="indexterm" mode="index-symbol-div">
  ...
  <xsl:apply-templates select="key('letter', $key)                                [generate-id(.) = generate-id(key('primary', normalize-space(concat(primary/@sortas, &#34; &#34;, primary)))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]" mode="index-primary">
    <xsl:with-param name="position" select="position()"/>
    <xsl:with-param name="scope" select="$scope"/>
    <xsl:with-param name="role" select="$role"/>
    <xsl:with-param name="type" select="$type"/>
    ...
    <xsl:when test="$refs/secondary or $refs[not(secondary)]/*[self::seealso]">
      <dd>
        <dl>
          ...
          <xsl:variable name="secondaries" select="$refs[secondary and                          generate-id() = generate-id(key('secondary', concat($key, &#34; &#34;, normalize-space(concat(secondary/@sortas, &#34; &#34;, secondary))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]"/>
...
<xsl:template match="indexterm" mode="index-secondary">
  ...
  <dt>
    ...
    <xsl:if test="$refs[not(tertiary)]/*[self::see]">
      <xsl:apply-templates select="$refs[generate-id() = generate-id(key('see', concat(normalize-space(concat(primary/@sortas, &#34; &#34;, primary)), &#34; &#34;, normalize-space(concat(secondary/@sortas, &#34; &#34;, secondary)), &#34; &#34;, &#34; &#34;, see))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]" mode="index-see">
        <xsl:with-param name="position" select="position()"/>
        <xsl:with-param name="scope" select="$scope"/>
        <xsl:with-param name="role" select="$role"/>
        <xsl:with-param name="type" select="$type"/>
  ...
  <xsl:choose>
    <xsl:when test="$refs/tertiary or $refs[not(tertiary)]/*[self::seealso]">
      <dd>
        <dl>
          <xsl:apply-templates select="$refs[generate-id() = generate-id(key('see-also', concat(normalize-space(concat(primary/@sortas, &#34; &#34;, primary)), &#34; &#34;, normalize-space(concat(secondary/@sortas, &#34; &#34;, secondary)), &#34; &#34;, &#34; &#34;, seealso))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]" mode="index-seealso">
            <xsl:with-param name="position" select="position()"/>
            <xsl:with-param name="scope" select="$scope"/>
            <xsl:with-param name="role" select="$role"/>
            <xsl:with-param name="type" select="$type"/>
          ...
          <xsl:variable name="tertiaries" select="$refs[tertiary and                          generate-id(.) = generate-id(key('tertiary', concat($key, &#34; &#34;, normalize-space(concat(tertiary/@sortas, &#34; &#34;, tertiary))))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]"/>
...
<xsl:template match="indexterm" mode="index-tertiary">
  ...
  <dt>
    ...
    <xsl:if test="$refs/see">
      <xsl:apply-templates select="$refs[generate-id() = generate-id(key('see', concat(normalize-space(concat(primary/@sortas, &#34; &#34;, primary)), &#34; &#34;, normalize-space(concat(secondary/@sortas, &#34; &#34;, secondary)), &#34; &#34;, normalize-space(concat(tertiary/@sortas, &#34; &#34;, tertiary)), &#34; &#34;, see))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]" mode="index-see">
        <xsl:with-param name="position" select="position()"/>
        <xsl:with-param name="scope" select="$scope"/>
        <xsl:with-param name="role" select="$role"/>
        <xsl:with-param name="type" select="$type"/>
  ...
  <xsl:choose>
    <xsl:when test="$refs/seealso">
      <dd>
        <dl>
          <xsl:apply-templates select="$refs[generate-id() = generate-id(key('see-also', concat(normalize-space(concat(primary/@sortas, &#34; &#34;, primary)), &#34; &#34;, normalize-space(concat(secondary/@sortas, &#34; &#34;, secondary)), &#34; &#34;, normalize-space(concat(tertiary/@sortas, &#34; &#34;, tertiary)), &#34; &#34;, seealso))[count(ancestor::node()|$scope) = count(ancestor::node()) and ($role = @role or $type = @type or (string-length($role) = 0 and string-length($type) = 0))])]" mode="index-seealso">
            <xsl:with-param name="position" select="position()"/>
            <xsl:with-param name="scope" select="$scope"/>
            <xsl:with-param name="role" select="$role"/>
            <xsl:with-param name="type" select="$type"/>
