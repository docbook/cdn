...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
		...
		exclude-result-prefixes="dbs xlink"
...
<xsl:template match="info">
  <xsl:apply-templates select="title|titleabbrev|subtitle|author|authorgroup/author"/>
</xsl:template>

<xsl:template match="title|titleabbrev">
  <xsl:if test="not(self::title) or (not(preceding-sibling::titleabbrev) and not(following-sibling::titleabbrev))">
...
<xsl:template match="author">
  <h3 class="author"><xsl:apply-templates select="personname|orgname"/></h3>
  <h4 class="email"><xsl:apply-templates select="email"/></h4>
  <xsl:if test="affiliation">
    <h4 class="affiliation"><xsl:value-of select="affiliation"/></h4>
  </xsl:if>
</xsl:template>
...
<xsl:template match="bibliosource" mode="bibliography.mode">
  <span>
...
<xsl:template name="slide.copyright">
  <div class="copyright">
    ...
    <xsl:text>&#xa9;</xsl:text>
    <xsl:call-template name="gentext.space"/>
    <xsl:value-of select="/dbs:slides/info/copyright/year"/>
    <xsl:call-template name="gentext.space"/>
    <xsl:value-of select="/dbs:slides/info/copyright/holder"/>
  </div>
</xsl:template>
...
<xsl:template name="slide.pubdate">
  <div class="pubdate">
    ...
    <xsl:text>: </xsl:text>
    <xsl:value-of select="/dbs:slides/info/pubdate"/>
  </div>
</xsl:template>
...
<xsl:template match="/" mode="slide.footer.mode">
  <xsl:if test="($generate.copyright != 0) and /dbs:slides/info/copyright">
    <xsl:call-template name="slide.copyright"/>
  </xsl:if>
  <xsl:if test="($generate.pubdate != 0) and /dbs:slides/info/pubdate">
