...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/zh_tw", xdmp:function(xs:QName("dbl10n:zh_tw")))
    return $map
};

declare function dbl10n:zh_tw()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/zh_tw.xqy"),
 plugin:register(dbl10n:capabilities(),"zh_tw.xqy"))
