...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/tl", xdmp:function(xs:QName("dbl10n:tl")))
    return $map
};

declare function dbl10n:tl()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/tl.xqy"),
 plugin:register(dbl10n:capabilities(),"tl.xqy"))
