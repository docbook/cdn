...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/af", xdmp:function(xs:QName("dbl10n:af")))
    return $map
};

declare function dbl10n:af()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/af.xqy"),
 plugin:register(dbl10n:capabilities(),"af.xqy"))
