...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/ta", xdmp:function(xs:QName("dbl10n:ta")))
    return $map
};

declare function dbl10n:ta()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/ta.xqy"),
 plugin:register(dbl10n:capabilities(),"ta.xqy"))
