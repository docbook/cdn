...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/cy", xdmp:function(xs:QName("dbl10n:cy")))
    return $map
};

declare function dbl10n:cy()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/cy.xqy"),
 plugin:register(dbl10n:capabilities(),"cy.xqy"))
