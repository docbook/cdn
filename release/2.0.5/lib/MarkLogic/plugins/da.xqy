...
{
    let $map := map:map()
    let $_   := map:put($map, "http://docbook.org/localization/da", xdmp:function(xs:QName("dbl10n:da")))
    return $map
};

declare function dbl10n:da()
as element(l:l10n)
{
  let $l10n := document {
...
( xdmp:log("Register docbook/da.xqy"),
 plugin:register(dbl10n:capabilities(),"da.xqy"))
